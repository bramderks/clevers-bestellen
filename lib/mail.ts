...
body{margin:0;padding:24px;background:#f4f4f4;font-family:Arial,sans-serif;color:#333}.container{max-width:700px;margin:auto;background:#fff;border:1px solid #e5e5e5;border-radius:10px;overflow:hidden}.header{background:#009640;color:#fff;padding:24px}.content{padding:24px}table{width:100%;border-collapse:collapse;margin-top:20px}th{background:#009640;color:#fff;padding:10px}td{padding:8px;border-bottom:1px solid #e5e5e5}.samenvatting{margin-top:24px;background:#f7f7f7;padding:16px;border-radius:8px}.footer{background:#fafafa;padding:18px;text-align:center;font-size:13px;color:#777}
...
export async function verstuurBestelMail(
  ...
  const { data, error } = await getResend().emails.send({
    ...
    html: maakHtml(vestiging, medewerker, datum, regels),
    attachments: [
      {
